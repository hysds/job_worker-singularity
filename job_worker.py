...
def run_job(job, queue_when_finished=True):
    ...
    try:
        ...
        if image_name is not None:
            logger.info("1st calling ensure_image_loaded(), image_name: %s" % image_name)
        ...
        for i, dep_img in enumerate(job.get('dependency_images', [])):
            logger.info("2nd calling ensure_image_loaded(), image_name: %s" % dep_img['container_image_name'])
        ...
        if all(pre_processor_sigs):
            ...
            # kill it for debugging purpose
            ### logger.info('****** killed for debugging purpose')
            ### sys.exit('****** killed for debugging purpose')
